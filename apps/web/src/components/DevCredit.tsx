...
const DEV = {
  ...
  linkedin: "https://www.linkedin.com/in/carforck", // confirmar/ajustar
  email: "carforck@gmail.com",
};

export function DevCredit({ className = "" }: { className?: string }) {
  return (
    <div className={`flex flex-col items-center gap-1.5 text-[12px] text-[var(--color-ink-soft)] ${className}`}>
      <span>
        Desarrollado por <span className="font-medium text-[var(--color-ink)]">{DEV.name}</span>
      </span>
