...
export async function PATCH(req: Request) {
  ...
  if (!b.id || typeof b.active !== "boolean") return NextResponse.json({ error: "datos inválidos" }, { status: 400 });
  const { error } = await supabase.from("recurrences").update({ active: b.active }).eq("id", b.id).eq("user_id", user.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ ok: true });
}
