/**
 * Descripción corta de cada sección de la app (qué es y para qué sirve).
 * Fuente única: se usa en el menú (tooltip) y como banner informativo en cada vista.
 */
export const SECTION_DESC: Record<string, string> = {
  "/dashboard": "Tu foto financiera de hoy: patrimonio, ingresos, gastos y alertas en un solo vistazo.",
  "/dashboard/novedades": "Lo nuevo de PlaticApp y todo lo que la app puede hacer por ti.",
  "/dashboard/movimientos": "Todo lo que entra y sale. Aquí buscas, filtras y editas cada gasto o ingreso.",
  "/dashboard/cuentas": "Tus cuentas: bancos, efectivo y billeteras. Tu patrimonio es la suma de sus saldos.",
  "/dashboard/deudas": "Quién te debe y a quién le debes. Márcalas como saldadas cuando se paguen.",
  "/dashboard/presupuestos": "Pones un límite de gasto por categoría y te avisamos cuando te acercas a él.",
  "/dashboard/recurrentes": "Pagos fijos como arriendo, suscripciones o sueldo. Te recordamos 1 día antes.",
  "/dashboard/recibos": "Las fotos de recibos que le mandas al bot quedan aquí guardadas como respaldo.",
  "/dashboard/inversiones": "Tus aportes e inversiones, y cómo van creciendo tus cuentas de inversión.",
  "/dashboard/categorias": "Las etiquetas con las que clasificas tus movimientos (comida, transporte, etc.).",
  "/dashboard/perfil": "Tus datos, la vinculación con Telegram y las opciones de tu cuenta.",
};
