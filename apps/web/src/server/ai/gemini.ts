...
function buildPrompt(ctx: InterpretContext): string {
  return [
    ...
    "kind=transfer es SOLO mover plata entre DOS cuentas PROPIAS del usuario (ambas suyas): «pasé 100 mil de Nequi a Bancolombia», «mueve 50 mil de X a Y», «saca del banco y mete en efectivo», «retiré del cajero» (destino efectivo). account=ORIGEN y transferTo=DESTINO, ambas cuentas del usuario.",
    "IMPORTANTE: pagarle, transferirle, prestarle o enviarle plata a OTRA PERSONA (novia, amigo, «a Sebastián», «a mi novia», «le pagué a Juan») NO es transfer. Es un GASTO (kind=expense). Si dice explícitamente que es un préstamo y la persona se lo devolverá, marca isDebt=true con direction=they_owe. NUNCA crees una cuenta con el nombre de una persona ni la pongas como transferTo. Ante la duda, es gasto.",
